...
func _init_connections() -> void:
	...
	if not _scenes_ui.is_connected("item_selected",_on_item_scene_selected):
		assert(_scenes_ui.connect("item_selected",_on_item_scene_selected) == OK)
	if not _node.is_connected("scene_selection_changed",_on_scene_selection_changed):
		assert(_node.connect("scene_selection_changed",_on_scene_selection_changed) == OK)
	if not _actors_ui.is_connected("item_selected",_on_item_actor_selected):
		assert(_actors_ui.connect("item_selected",_on_item_actor_selected) == OK)
	if not _node.is_connected("actor_selection_changed",_on_actor_selection_changed):
		assert(_node.connect("actor_selection_changed",_on_actor_selection_changed) == OK)
	if not _add_ui.is_connected("pressed",_on_add_sentence_pressed):
		assert(_add_ui.connect("pressed",_on_add_sentence_pressed) == OK)
	if not _node.is_connected("sentence_added",_on_sentence_added):
		assert(_node.connect("sentence_added",_on_sentence_added) == OK)
	if not _node.is_connected("sentence_removed",_on_sentence_removed):
		assert(_node.connect("sentence_removed",_on_sentence_removed) == OK)
	if not _textures_ui.is_connected("item_selected",_on_item_textures_selected):
		assert(_textures_ui.connect("item_selected",_on_item_textures_selected) == OK)
	if not _node.is_connected("texture_selection_changed",_on_texture_selection_changed):
		assert(_node.connect("texture_selection_changed",_on_texture_selection_changed) == OK)
	if not _view_ui.is_connected("pressed",_on_view_pressed):
		assert(_view_ui.connect("pressed",_on_view_pressed) == OK)
	if not _node.is_connected("view_selection_changed",_on_view_selection_changed):
		assert(_node.connect("view_selection_changed",_on_view_selection_changed) == OK)
	if not _node.is_connected("sentence_event_visibility_changed",_on_sentence_event_visibility_changed):
		assert(_node.connect("sentence_event_visibility_changed",_on_sentence_event_visibility_changed) == OK)
	if not _node.is_connected("sentence_selection_changed",_on_sentence_selection_changed):
		assert(_node.connect("sentence_selection_changed",_on_sentence_selection_changed) == OK)
