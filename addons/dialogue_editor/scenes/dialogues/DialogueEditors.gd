...
func _init_connections() -> void:
	if not _data.is_connected("dialogue_view_selection_changed", _on_dialogue_view_selection_changed):
		assert(_data.connect("dialogue_view_selection_changed", _on_dialogue_view_selection_changed) == OK)
...
func _draw_nodes_view() -> void:
	var nodes_ui = DialogueNodesView.instantiate()
	add_child(nodes_ui)
	nodes_ui.set_data(_dialogue, _data)

func _draw_bricks_view() -> void:
	var bricks_ui = DialogueBricksView.instantiate()
	add_child(bricks_ui)
